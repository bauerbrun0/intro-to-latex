...
\title{
	\vspace{5cm}
	\Huge\textbf{Szakmai Gyakorlat} \\
    \Huge\textbf{Munkanapló} \\
    \vspace{1cm}
    \Large \textit{Bauer Brúnó Csaba} \\
    \vspace{0cm}
    \large \textit{QE12MB} \\
    \vspace{0.5cm}
    \large \textit{Seiren Hungary Kft.} \\
    \vspace{0.5cm}
    \normalsize \textit{2023. 09. 04. - 2023. 11. 10.}
}
\date{2024. 12. 30.}

% Chapter formatting (no chapter numbering)
...
% Blank page befor TOC
\newpage
\null
\thispagestyle{empty}
\addtocounter{page}{-1} % Prevent the blank page from incrementing the page number
\newpage
...
\begin{minted}[fontsize=\small, bgcolor=lightgray]{typescript}
bind(dnOrSaslMechanism, [password], [controls])
\end{minted}

A bind függvény egy bind operációt kezdeményez az AD szerver felé. Ezzel az operációval
...
Ezek után a projekt megkezdéséhez szükséges, kezdetleges terveket készítettem el. (Ábra \ref{fig:login_diagram}, \ref{fig:further_requests_diagram})\\
\begin{figure}[ht]
  \centering
...
tartalmaz egy fejlécet, egy fő tartalmi részt (body), és egy láblécet. (Ábra \ref{fig:dark_light_modes})\\
...
\begin{minted}[fontsize=\small, bgcolor=lightgray]{text}
// pnpm run test:unit
...
A Header komponenst implementáltam először (Ábra \ref{fig:header_component}). Sikerült teljesen reszponzívra megírni (Ábra \ref{fig:header_component_mobile}). Tartalmaz
...
Ezt követően a bejelentkezési oldal frontend részét csináltam meg (Ábra \ref{fig:login_page}).
...
A bejelentkezés backend részét implementáltam (Ábra \ref{fig:login}), valamint – a minden
...
A mai napot a profil infó oldal (Ábra \ref{fig:profile_info}) implementálásával kezdtem. Itt tekinthetik meg a
...
Az EntraPass által generált riportok formája a \ref{fig:csv_format}. ábrán látható.
Ebből a formátumból kell majd a \ref{fig:card_usages_schema}. ábrán megjelölt séma szerint menteni az adatokat az adatbázisba. Idő közben a követelmények bővültek
...
hogy a könyvtárhoz tartozó CSS fájlokat felülírtam a saját CSS fájljaimmal. A végeredmény a \ref{fig:calendars}. ábrán
látható.\\
...
gombra kattintva meghív (Ábra \ref{fig:timesheet_search}). E mellett az oldal PageLoad függvényét is megírtam, hogy az első
...
nézetben (Ábra \ref{fig:production_view}) a komponens lenyitható füleket tartalmaz, míg irodai nézet esetében (Ábra \ref{fig:office_view}) a komponens csak
egyszintes.
...
lehet megtekinteni (Ábra \ref{fig:card_usages_table}).
...
munkaidő adatokhoz (\inltxt{Office|ProductionShiftViewData}) tartozó statisztikák (Ábra \ref{fig:stat_cards}). Például:
...
diagramot. Ezen a diagramon (Ábra \ref{fig:chart}.) napokra bontva szerepel az összes dolgozott idő, a túlóra, valamint -
...
segítségével (Ábra \ref{fig:export_component}), hogy az exportált pdf (Ábra \ref{fig:report_pdf}) tartalmazza e a
...
\href{https://www.npmjs.com/package/exceljs}{exceljs} könyvtár segítségével (Ábra \ref{fig:report_xlsx}).
...
Elkészítettem a dolgozók táblázatát (Ábra \ref{fig:employees}.) tartalmazó \inltxt{/admin/employees} oldalt. A táblázat fejlécében a
...
középső a dolgozó profiloldalára navigál, míg a jobb szélső gomb a dolgozó adatainak törlésére (Ábra \ref{fig:delete_employee}) szolgál.
...
Létrehoztam a kártyahasználati adatokat tartalmazó táblázatot (Ábra \ref{fig:admin_card_usages}), amely hasonló a dolgozók
...
A nap végére sikerült befejezni a törlés funkciót (Ábra \ref{fig:delete_card_usage}). \\
...
Ezért létrehoztam egy saját \inltxt{TimeInput} komponenst (Ábra \ref{fig:time_input}), amely lehetővé teszi a másodperc értékének
megadását is.
...
törlését implementáltam frontend-, és backendoldalon. (Ábra \ref{fig:manage_card_usages}.)
...
Először létrehoztam a \inltxt{SupervisementGroups} adatbázis sémát (Ábra \ref{fig:sup_groups_schema}.), majd megírtam a supervisement
...
implementálni (Ábra \ref{fig:sup_groups}.), amely egy táblázatot tartalmaz a létrehozott supervisement csoportokról.
...
Ezt követően létrehoztam a \inltxt{SupervisementGroupMemberships} adatbázis sémat (Ábra \ref{fig:sup_schema}), amely a
...
tartalmaz (Ábra \ref{fig:sup_edit}). A bal oldali táblázat tartalmazza azokat a dolgozókat, akik még nincsenek hozzáadva a csoporthoz. Az
...
Befejeztem az /admin/supervisement-groups/[id] oldal implementálását (Ábra \ref{fig:sup_edit_final}.).
...
implementáltam (Ábra \ref{fig:groups_pages}). A \inltxt{/supervisor/groups} oldalon a felettes megtekintheti a saját felügyelt
...
A mai napon a főoldalt, valamint a hiba oldalakat implementáltam (Ábra \ref{fig:home_page}, \ref{fig:error_page}).
...
\FloatBarrier
\begin{minted}[fontsize=\small, bgcolor=lightgray]{bash}
